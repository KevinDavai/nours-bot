...
export class Queue extends Array {
     ...
     */
    public add(track: Track, offset: number = 0): void {
        if (!(track instanceof this.erela.track)) {
            throw new RangeError("Queue#add(track: Track) Track must be extended from \"Track\".");
        }

        if (isNaN(offset)) {
            throw new RangeError("Queue#add(track: Track, offset: number) Offset must be a number.");
        }

        if (offset < 0 || offset > this.size) {
            // tslint:disable-next-line: max-line-length
            throw new RangeError(`Queue#add(track: Track, offset: number) Offset must be or between 0 and ${this.size}.`);
        }

...
module.exports = async (client, GuildMember, message) => {
       const player = client.music.players.get(message.guild.id);
        ...
        if (!GuildMember.guild.me.voice.channel) { // Check if bot is in a channel if not return
            return
        }
    
        const NumberChannel = GuildMember.guild.me.voice.channel.members.size
        let timeout
        ...
        if (NumberChannel < 2) { // If bot is alone
            timeout = setTimeout(() => {
                    if (GuildMember.guild.me.voice.channel.members.size < 2) {
